const UI = (() => {
  ...
  function renderInstitutionalLayer(data) {
    ...
    if (reports) {
      ...
      reports.innerHTML = `<div class="card-header"><span class="card-title">Institutional Reports</span></div>${reps.map(r => `<div style="padding:8px;border-bottom:1px solid var(--border-color)"><b>${r.title}</b><button class="btn btn-secondary" style="float:right" onclick="navigator.clipboard && navigator.clipboard.writeText(${JSON.stringify('${JSON.stringify(r)}')})">Copy</button><div style="font-size:11px;color:var(--text-muted)">${(r.sections || []).map(s => s.title).join(' · ')}</div></div>`).join('') || '<div class="empty-state-text">No reports</div>'}`;
    }
  }
